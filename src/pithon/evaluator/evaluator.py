...
def initial_env() -> list[tuple[str, EnvValue]]:
    # Insert primitive functions into the environment
    return [
        ('+', primitive_add),
        ('-', primitive_sub),
        ('*', primitive_mul),
...
def evaluate_stmt(node: PiStatement, env: list[tuple[str, EnvValue]]) -> EnvValue:
    ...
    if isinstance(node, PiNumber):
        ...
        closure = FunctionClosure(node, env.copy())
        ...
        closure_env = func_val.closure_env.copy()
        call_env = closure_env + []
        for i, arg_name in enumerate(funcdef.arg_names):
            if i < len(args):
                insert(call_env, arg_name, args[i])
            else:
                raise TypeError("Argument manquant pour la fonction.")
        if funcdef.vararg:
            insert(call_env, funcdef.vararg, args[len(funcdef.arg_names):])
