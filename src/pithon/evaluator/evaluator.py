...
from pithon.evaluator.envvalue import EnvValue, FunctionClosure, VList, VNone, VTuple, VNumber, VBool, VString
...
def evaluate_stmt(node: PiStatement, env: EnvFrame) -> EnvValue:
    ...
    if isinstance(node, PiNumber):
        ...
        if not isinstance(operand, VBool | VNumber | VString | VNone | VList | VTuple):
            raise TypeError(f"Type non supporté pour l'opérateur 'and': {type(operand).__name__}")
        return VBool(not operand.value)

    elif isinstance(node, PiAnd):
        left = evaluate_stmt(node.left, env)
        if not isinstance(left, VBool | VNumber | VString | VNone | VList | VTuple):
            raise TypeError(f"Type non supporté pour l'opérateur 'and': {type(left).__name__}")
        if not left.value:
            return left
        right = evaluate_stmt(node.right, env)
        if not isinstance(right, VBool | VNumber | VString | VNone | VList | VTuple):
            raise TypeError(f"Type non supporté pour l'opérateur 'and': {type(right).__name__}")
        return right

    elif isinstance(node, PiOr):
        left = evaluate_stmt(node.left, env)
        if not isinstance(left, VBool | VNumber | VString | VNone | VList | VTuple):
            raise TypeError(f"Type non supporté pour l'opérateur 'or': {type(left).__name__}")
        if left.value:
            return left
        right = evaluate_stmt(node.right, env)
        if not isinstance(right, VBool | VNumber | VString | VNone | VList | VTuple):
            raise TypeError(f"Type non supporté pour l'opérateur 'or': {type(right).__name__}")
        return right
        ...
        for item in iterable:
            env.insert(node.var, item)  # Les boucles python ne créent pas de nouvel environnement
            try:
                last_value = evaluate(node.body, env)
            except BreakException:
                break
